...
//根据传入的父Id，返回下级Id（包括自己的Id）
function getSubordinateIdByPid($pid){
	...
	public function memberListAjax(){
		...
		$customer = input('customer','','trim,strip_tags');
		$adminIdList = $this -> getSubordinateIdByPid($customer); //下级Id数组
		...
		$list = Db::name('member')
			->field('s.name as scenarios_name,c.task_name,g.name,m.review,m.uid,m.username,m.nickname,m.mobile,m.reg_time,m.level,m.status,m.uid,m.duration,m.last_dial_time,m.semantic_label,m.flow_label,m.call_times,m.affirm_times,m.negative_times,m.neutral_times,m.effective_times,m.hit_times,m.knowledge_label')
			->alias('m')
			->where($where)
			->where($subWhere)
  ...
  public function backdetail(){
    	...
    	if($froms == 'record'){
			...
			->where('mobile',$mobile)->where('task',$taskId)->find();
			if (!$memberInfo){
				$memberInfo['uid'] = '--';
				$memberInfo['nickname'] = '--';
				$memberInfo['sex'] = '--';
			...
			$Info = Db::name('tel_call_record')
				->field('mobile,status,level,duration,last_dial_time,call_id,call_times,originating_call,record_path')
				->where('id',$recordId)->find();
		...
		if ($memberInfo['sex'] > 0){
			$memberInfo['sex'] = $memberInfo['sex']==1?'女':'男';
		}
		else{
			$memberInfo['sex'] = '未知';
		}
    ...
    public function editMember(){
    	if(IS_POST){
			...
			$memberInfo = Db::name('member')->where(array('owner'=>$uid, 'user_type'=>1, 'mobile'=>$ctype['mobile']))->find();
			if ($memberInfo){
				
				return returnAjax(1,'该号码已标记黑名单！');
			}
    ...
    public function addMember(){
    	...
    	if(IS_POST){
				...
				$ctype['nickname'] = input('nickname');
				$ctype['group_id'] = input('groupId','0','trim,strip_tags');
				$ctype['username'] = input('nickname','','trim,strip_tags');
				$ctype['sex'] = input('sex');  //0是男  1是女
				$ctype['mobile'] = input('phonenumber','','trim,strip_tags');
    ...
    public function importExcel(){
		...
		//获取黑名单
		$backMembers = Db::name('member')->field('mobile')
					->where(array('owner'=>$uid, 'user_type'=>1))
					->select();
					
		$backMemberList = array();
		if ($backMembers){			
			$backMemberList = array_column($backMembers, 'mobile');		
		}			
		$data = array();
		$taskdata = array();
		$numlist = array();
		...
		\think\Log::record('@####taskdatanum='.$long);
		foreach ( $excelArr as $k => $v ){
			...
			$user['company'] = trim($v[3]);
			
			$user['mobile'] = trim(preg_replace("/(\s|\&nbsp\;|　|\xc2\xa0)/","",$v[1]));
								
			$isMob="/^1[2345789]{1}\d{9}$/";
			$isTel="/^([0-9]{3,4}-)?[0-9]{7,8}$/";
			if(!preg_match($isMob,$user['mobile']) && !preg_match($isTel,$user['mobile'])){
				if($successCnt == 100 || ($k+1) == $long){
					$this->saveImportData($where, $numlist, $data, $taskdata);
					$successCnt = 0;
					$data = array();
					$numlist = array();
					$taskdata = array();
			...
			if (in_array($user['mobile'], $backMemberList)){
				if($successCnt == 100 || ($k+1) == $long){
					$this->saveImportData($where, $numlist, $data, $taskdata);
					$successCnt = 0;
					$data = array();
			...
			if (!in_array($user['mobile'], $numlist)){
				$user['reg_time'] = time();
				$user['salt'] = rand_string(6);
				$user['password'] = md5(substr($user['mobile'],5).$user['salt']);
				if ($taskId){
					$user['task'] = $taskId;
					$user['status'] = 1;
					if ($telConfig){
	...
	private function saveImportData($where, $numlist, $data, $taskdata){
		...
		if ($taskdata){
			...
			$ret = $this->connect->table('autodialer_number_'.$where['task'])->insertAll($taskdata);
		}
		...
		if (!$super) {
         ...
         if($flag == "join"){
				...
				if (is_array($idlist) && count($idlist)) {
					\think\Log::record('idlist='.json_encode($idlist));
					$cwhere["uid"] = ["in",$idlist];
    ...
    public function group(){
		...
		$list = Db::name('member_group')->where('owner','in', $IdList)->where($where)->paginate(5, false, array('query'  =>  $this->param));
	...
	public function callLog(){
		...
		$status = input('status','','trim,strip_tags');
		$timelong = input('timelong','','trim,strip_tags');
		$level = input('level','','trim,strip_tags');
		$ready = input('ready','','trim,strip_tags');
		...
		if($startDate && $endTime){
			$where['last_dial_time'] = ["between",[strtotime($startDate),strtotime($endTime)]]; 
		}
		...
		$adminInf =  Db::name('admin')->field('user_type')->where("id",$uid)->find();
		if($adminInf["user_type"] == 1){
		...
		foreach($list as $key => $item){
			...
			$tlist = Db::name('tel_config')->field('task_name,type')->where("task_id",$item['task_id'])->find();
			if($tlist){
				$list[$key]['task_name'] = $tlist['task_name'];
			}
			
			
			$list[$key]['call_type'] = $tlist['type']?'呼入':'外呼';
    ...
    public function whitelist(){
		...
		$IdList = getSubordinateId(); //下级Id数组	
	  	...
	  	$list = Db::name('member')
			...
			->alias('m')
			->where($where)->where('m.owner','in', $IdList)
			->join('member_group g','g.id = m.group_id','LEFT')
...
public function redial(){
	...
	$taskdata = array();
	$taskdata['number'] = input('mobile','','trim,strip_tags');
	$ret = $connect->table('autodialer_number_'.$task)->insertGetId($taskdata);
	if ($ret){
		return  returnAjax(0,'已成功添加到重拨对列');
	}
	else{
		
		return  returnAjax(1,'重拨失败');
	}
...
function exportmemberExcel()
{
	...
	public function delAll(){
		...
		$list = Db::name('member')->field('mobile')->where($where)->where($subWhere)->select();
		    	
		$back = array();    		
		foreach ($list as $k=>$v){

			if($v['mobile']){
    		   	array_push($back, $v['mobile']);
			}

		}

	
		$result = Db::name('member')->where($where)->where("owner",$uid)->where($subWhere)->delete();

	    if($result){

	    	Db::name('tel_call_record')->where("owner",$uid)->where("mobile","in",$back)->delete();
		
			Db::name('tel_bills')->where("phone","in",$back)->delete();
