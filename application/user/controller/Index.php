...
class Index extends User{
	...
	public function login($username = '', $password = '', $verify = ''){
		if (IS_POST) {
			...
			$user = Db::name('admin')->where('username',trim($username))->find();	
			if($user && $user['status']){
       ...
       /* 
		...
		/*	$res = Db::name('config')->where('group', 36)->find();
			...
			* */
			...
			// $res = db('site')->field('logo,name')->where('domain', $domainurl)->find();
		
			// if($res["logo"]){
	
				// $list = db('picture')->field('path')->where('id', $res["logo"])->find();
				// if($list['path']){
				   // $res['logo'] = $list["path"];
				// }
   ...
   //返回拨打状态 
	...
	public function levelData(){
		...
		if($time == 7){
			...
			$end = time();//strtotime(date("Y-m-d", strtotime("-1 day")));
	...
	public function getMyData()
	{
        ...
        $tsrtotal = Db::name('tsr_group')->alias('g')
				->join('tel_tsr t','g.id = t.tsr_group_id')
				->where('owner', $uid)->count(1);
