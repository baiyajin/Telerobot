...
class scenarios extends User{
	...
	public function getKeywordItem(){
		...
		public function learning(){
			...
			$timesSort = input('times_sort_flag', '','trim,strip_tags');
			...
			//SELECT count(t.hot) FROM ( SELECT count(1) as hot FROM `rk_tel_learning` WHERE  `status` = 0  AND `owner` = 5555 GROUP BY content ORDER BY hot desc) t
			if ($timesSort == 'true'){
				$count =  Db::name('tel_learning')->where($where)->count('DISTINCT content');
			}
			else{
				$count =  Db::name('tel_learning')->where($where)->count(1);
			}
		...
		public function backdetail(){
				...
				if($type == "scene"){
					...
					$num = Db::name('tel_call_record')->where('status',1)->where('call_id',$uid)->count("id");

					if ($memberInfo['call_id']){
						
						$bills = Db::name('tel_call_record')->where('call_id',$memberInfo['call_id'])->order('id asc')->select();
					}
					else{
						$bills = Db::name('tel_call_record')->where('phone',$memberInfo['mobile'])->order('id asc')->select();
					}
				}
				else{
    ...
    public function scene(){
		...
		$adminlist = Db::name('admin')->field('examine,user_type')->where('id', $uid)->find();
		$this->assign('examine', $adminlist['examine']);
		$this->assign('super', $super);
		$this->assign('userType', $adminlist['user_type']);
		...
		$list = Db::name('admin')->field("id,username,realname,user_type")->where('id','in', $IdList)->select();
		
		$this->assign('subordinate', $list);
	...
	public function getscene(){
			...
			$list = Db::name('tel_label')->field("id,label")->where($Label)->select();
	...
	public function addflowNote(){
			...
			if($flowId){
        		...
        		$res = Db::name('tel_scenarios_node')->field("name")
        		->where('scenarios_id', $data['scenarios_id'])->select();
        		
        		foreach ($res as &$v){
		        	if($v["name"] == $data['name']){
		        		return returnAjax(1,'同一场景下的节点名称不能重复。',$v["name"]);
		        	}
		        }
   ...
   //返回流程节点列表
    ...
    public function addThinkTank(){
			...
			$knbreak = input('knbreak','','trim,strip_tags');
			$data['break'] = 0;
			if($knbreak != ""){
				$data['break'] = $knbreak;
			}
			...
			if($data['keyword']){
				...
				$keyRes = Db::name('tel_knowledge')->field('keyword')->where($where)->select();
				
				$oldKey = array();				
				foreach ($keyRes as $kres => $resval) {
					
					if($resval['keyword']){
						
						$temp = explode(",",$resval['keyword']);
						foreach ($temp as $tkey => $tval) {
							array_push($oldKey,$tval);
						}
					}
					
				}

				foreach ($keylist as $kwkey => $kwvalue) {
					
					foreach ($oldKey as $okey => $ovalue) {
						
						if($kwvalue == $ovalue){
							array_push($repeat,$kwvalue); 
						}
			   ...
			   // return returnAjax(1,'新建失败',$repeat);
			...
			$backData = "";
			if(count($repeat)){
				$backData = "有重复的关键词，重复的关键词是：".$back;
			}

			return returnAjax(0,'保存成功！',$backData);
    ...
    public function getKnowledgeList()
    {
        ...
        $keyword = input('keyword','','trim,strip_tags');
        $kntypeword = input('kntypeword','','trim,strip_tags');

        if($kntypeword == "关键字" || $kntypeword == "" ){
			...
			$list = Db::name('tel_knowledge')->where($where)->order('sort desc')->page($page,$Page_size)->select();
	    	...
	    	if($keyword != ""){
				...
				$list = Db::name('tel_corpus')->field('src_id')->where($where)->select();
				
				$srcid = array();
				
				foreach($list as $keys => &$val){
					array_push($srcid, $val['src_id']);
					
				}
				
				$srcid = array_unique($srcid);
				sort($srcid);
			
				
				$list = Db::name('tel_knowledge')->where("id","in",$srcid)->order('sort desc')->page($page,$Page_size)->select();
	    	} 
			else{
				$list = Db::name('tel_knowledge')->where($where)->order('sort desc')->page($page,$Page_size)->select();
			}
		...
		foreach($list as $key => &$item){
			...
			$con = mb_strlen($item['keyword'],"utf-8");
			if($con > 0){
			...
			if($con > 40){
				$item['keyword'] = mb_substr($item['keyword'],0,40,"utf-8");
			...
			$tcwhere = array();
			if($sceneId != ""){
	    		$tcwhere["scenarios_id"] = $sceneId;
	  		}	    	
	  		$tcwhere["src_type"] = 1;
	    	$tcwhere["src_id"] = $item['id'];
			$tclist = Db::name('tel_corpus')->field('content')->where($tcwhere)->select();
			$snum = count($tclist);
			if($snum > 0){
				$item['speechcraft'] = $tclist[0]['content'];
				$item['snum'] = $snum;
			}
			else{
				$item['speechcraft'] = "";
     ...
     //删除知识库记录
  	...
  	public function getKnowledgeInfo()
  	{
		...
		$result = Db::name('tel_knowledge')->where('id', $noteId)->find();
    ...
    //复制话术
  	public function copyScene()
  	{
  		  ...
  		  $memberId = input('memberId','','trim,strip_tags');	
	      ...
	      $scenarios = Db::name('tel_scenarios')
				   ->field('name,member_id,type,is_tpl,status,break,auditing')
	  			   ->where('id',$sceneId)->find();

			$tscen = array();
			$tscen['name'] = $newSName;
			$tscen['member_id'] = $memberId;
			...
			$newId = Db::name('tel_scenarios')->insertGetId($tscen);
                     ...
                     //复制流程节点与流程
         	$result = Db::name('tel_scenarios_node')->field('id,name,sort,type')->where('scenarios_id', $sceneId)->select();	
	        ...
	        foreach ($result as $key => $value) {
				...
				foreach ($flowList as $fnkey => $fnval) {
					...
					if($fnval["type"] == 0){
						...
						$insertlabel['member_id'] = $memberId;
					...
					foreach ($nflowList as $okey => $oval) {
							...
							if($oval["type"] == 1 && $oval["action"] == 2 && $oval["action_id"]){
							    ...
							    if($oval["action_id"] > 0){
									...
									if(isset($tsarray[$oval["action_id"]])){
			...
			//复制知识库
			$tfnw = array();
			$tfnw['scenarios_id'] = $sceneId;
				

			$knresult = Db::name('tel_knowledge')
			 ->field('id,scenarios_id,name,type,keyword,keyword_py,action,action_id,intention,is_default')
			 ->where($tfnw)
			 ->select();
			...
			if($newId){
				return returnAjax(0,'复制成功',$newId);
			}else{
				return returnAjax(1,'复制失败');
			}
    ...
    public function telflowNode()
    {
		...
		$pauseTime = input('pauseTime','5000','trim,strip_tags');
    ...
    public function getFnodeInfo()
    {
		...
		$tfnw['id'] = $flowId;
	 	...
	 	$result = Db::name('tel_flow_node')->field('name,break,action,action_id,flow_label,pause_time,no_speak_knowledge_id,sms_template_id,bridge')->where($tfnw)->find();
				...
				if ($nval['isDelete'] == 1) {
					if($nval["id"] > 0){
						...
						if($nval["pid"] > 0){
							...
							$count = Db::name('tel_flow_node')->where("id",$nval["pid"])->count(1);
							if($count){
								$flow['pid'] = $nval["pid"]; 
							}else{
								$flow['pid'] = 0; 
						...
						if($nval["type"] == 'Menu' && count($nval["thumb"]) && $nval["id"] > 0){
							   ...
							   foreach ($nval["thumb"] as $tkey => $tval) {
									...
									//if($tval["nextNodeId"]){
									if(isset($tval["nextNodeId"]) && $tval["nextNodeId"] > 0){
	...
	//返回录音流程记录信息
  	public function getsflowInfo()
  	{
		 $noteId = input('id','','trim,strip_tags');		
			...
			$nodeInfo = Db::name('tel_flow_node')->field('name')->where("id", $noteId)->find();
			if (!$nodeInfo){ 
				return returnAjax(1,'暂没有数据');
			}
			 
			$corpus = Db::name('tel_corpus')->field('id,content,audio')->where(array("src_id"=>$noteId,'src_type'=>0))->find();
			if (!$corpus){
				return returnAjax(1,'暂没有数据');
			}	
			$corpus['name'] = $nodeInfo['name'];
		
		 
		$vars = $this->matchVar($corpus['content']);
		if (is_array($vars)){//有变量
			$corpus['isVar'] = 1;
			if (empty($corpus['audio'])){//audio空
				$corpus['audio'] = $vars;
			}
			else{//audio不空
				foreach($vars as $k=>&$var){
					...
					if ($this->is_serialized($corpus['audio'])){
						$oldVars = unserialize($corpus['audio']);
						$var['audio'] = "";
						foreach($oldVars as $oldVar){
							if (isset($oldVar['txt']) && $var['txt'] == $oldVar['txt']){
								//$var[$k]['url'] = $oldVar['audio'];
								$var['audio'] = $oldVar['audio'];
								break;
							}
						}
					}
					else{
						$var['audio'] = $corpus['audio'];
						break;
					}
				}
			...
			$corpus['isVar'] = 0;
			//audio空 或 序列化
			if (empty($corpus['audio']) || $this->is_serialized($corpus['audio'])){
				
				$oldVars = unserialize($corpus['audio']);
				
				/*foreach($oldVars as $oldVar){
					if (isset($oldVar['txt']) && isset($oldVar['audio']) ){
						$var['audio'] = $oldVar['audio'];
						break;
					}
				}*/
				
				$corpus['audio'] = $oldVars[0]['audio'];
			}
		}
			
		return returnAjax(0,'success', $corpus);
  	}
    ...
    public function addNodeSound(){
		...
		$data = array();
		$audio = array();
		$oldAudio = array(); //单个的时候
		$audioUrl = "";
		
		$temp = array();
		$temp['sid'] = input('sid/a','','trim,strip_tags');
 		$temp['text'] = input('text/a','','trim,strip_tags');
 			
 		$return = Db::name('tel_corpus')->field('content,audio')->where('id', $temp['sid'][0])->find();
		$vars = $this->matchVar($return["content"]);
		if($return["audio"] && $this->is_serialized($return["audio"])){				
			$oldAudio = unserialize($return['audio']);
		}
		...
		if (is_array($vars)){
			foreach($vars as $k=>$var){
				...
				if ($var['type'] == 1){
					$arr['txt'] = ltrim($var['txt'],'[');
					$arr['txt'] = rtrim($arr['txt'],']');
					$arr['type'] = 1;
				}
				else{
					if(!empty($_FILES['update-audio-file-'.$fileIndex]['tmp_name'])){
						
						$tmp_file = $_FILES ['update-audio-file-'.$fileIndex]['tmp_name'];
						if($tmp_file){
							$file_types = explode ( ".", $_FILES ['update-audio-file-'.$fileIndex]['name'] );
							$file_type = $file_types [count ( $file_types ) - 1];
			...
			$data['audio'] = serialize($audio);
		}
		else{
			$tmp_file = $_FILES ['update-audio-file-0']['tmp_name'];
		
			if($tmp_file){
				
				$file_types = explode ( ".", $_FILES ['update-audio-file-0']['name'] );
				$file_type = $file_types [count ( $file_types ) - 1];
		...
		$ret = Db::name('tel_corpus')->where('id', $temp['sid'][0])->update($data);
		if ($ret !== false){
			return returnAjax(0,'成功', $audioUrl);
		}
		else{
			...
			foreach ($excelArr8 as $key8 => $value8){
				...
				foreach ($value8 as $k8 => $v8) {
					...
					if($excelArr8[0][trim($k8)] != "id"){
						...
						if($excelArr8[0][trim($k8)] == "status"){
							$temp[$excelArr8[0][trim($k8)]] = (int)trim($v8);
						}
					}
	...
	public function changeSort(){
	        ...
	        $data['sort'] = input('nowsort','','trim,strip_tags');		
			$showId = input('showId','','trim,strip_tags');
			

			$result = Db::name('tel_knowledge')->where('id', $showId)->update($data);
