...
function sendTemplateMsg($touser, $template_id, $url='', $data, $topcolor = '#173177',$wxInfo=""){
    ...
    if (!$wxInfo){
  	...
  	\think\Log::record('sendTemplateMessage failure='.json_encode($ret));
}
...
function sendKeFuMsg($wxInfo, $openId, $msg){		
	...
	function sendMsg($owner, $templateId, $mobile){
	   			
		$admin = db('admin')->field('credit_line,money')->where('id',$owner)->find();
		
		$smsTmple = db('sms_template')->field('price,name,conent,channel_id,sign_id,type')->where(array('id'=>$templateId,'status'=>1))->find();
		if (!$smsTmple){
			
			\think\Log::record('sendSms sms_template is null templateId='.$templateId);
			return false;
		
		}
		
		$smsSign = db('sms_sign')->field('name')->where(array('id'=>$smsTmple['sign_id'],'status'=>1))->find();
		if (!$smsSign){
			\think\Log::record('sendSms sms_sign is null sign_id='.$smsTmple['sign_id']);
			return false;
		}
		
		$money = $admin['money'] + $admin['credit_line'];
		 
		if($money > $smsTmple['price']){
			
			// $code = rand_string(3,1);
			// $content = $smsTmple["conent"];
			// $fullContent = "【".$smsSign['name']."】".$content;
			
			$smsChannel = db('sms_channel')->field('name,url,user_id,access_secret,password')->where('id',$smsTmple['channel_id'])->find();
			
			$smsConfig = [
				'accessKeyId' => $smsChannel ['user_id'],                 // your appid
				'accessKeySecret' => $smsChannel ['access_secret'],         // your app_secret
				'signName'    => $smsSign ['name'],                    // your 签名
				'templateCode' => $smsTmple ['name'],         // your 模板编号
			  
			];
			$client  = new \Flc\Dysms\Client($smsConfig);
			$sendSms = new \Flc\Dysms\Request\SendSms();
			$sendSms->setPhoneNumbers($mobile);
            $sendSms->setTemplateParam(array('code'=>'123456'));
            ...
            $ch = curl_init('http://test.izoov.com/zv-member/zv/msg/sendmsg?mobile='.$mobile
			...
			$out = curl_exec($ch);
			\think\Log::record('send sms res='.$out);
			curl_close($ch);
			...
			$data['owner'] = $owner;
			$data['client_ip'] = get_client_ip(0);
			if (isset($result['Code']) && $result['Code'] == 'OK'){
				$data['status'] = 1;			
				$ret = db('sms_record')->insertGetId($data);
				if(!$ret){
					\think\Log::record('send sms sms_record failure mobile='.$mobile);
					return false;
				}
				
				$ret = db('admin')->where('id', $data['owner'])->setDec('money', $smsTmple['price']);
				if (!$ret){
					\think\Log::record('send sms update admin tabile failure mobile='.$mobile);
					return false;
				}
			}
			else{
				$data['status'] = 0;
				\think\Log::record('send sms failure res='.var_dump($resp));
				$ret = db('sms_record')->insertGetId($data);
				if(!$ret){
					\think\Log::record('send sms sms_record failure mobile='.$mobile);
				}
			...
			\think\Log::record('sendSms balance is not enough owner='.$owner);
			return false;
		}
	}
