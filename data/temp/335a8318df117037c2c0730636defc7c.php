<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:57:"/www/wwwroot/web/application/user/view/sms/signature.html";i:1551953756;s:55:"/www/wwwroot/web/application/user/view/public/base.html";i:1569485441;}*/ ?>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8"/>
...
<div class="col-lg-12">
							...
							<?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?>
								<tr>
								 <td class="text-center">
										<input type="checkbox" name="roleids" class="rolecheck" value="<?php echo (isset($list['id']) && ($list['id'] !== '')?$list['id']:""); ?>"/>
								 </td>
									
									<td class="text-center"><?php echo (isset($list['id']) && ($list['id'] !== '')?$list['id']:""); ?></td>
									<td class="text-center"><?php echo (isset($list['name']) && ($list['name'] !== '')?$list['name']:""); ?></td>
									
									<td class="text-center"><?php echo (isset($list['create_time']) && ($list['create_time'] !== '')?$list['create_time']:""); ?></td>
									<td class="text-center">
									<?php if($isSuper == '1'): switch($list['status']): case "1": ?>
													<a href="javascript:void(0);" onclick="setstatus(<?php echo $list['id']; ?>,0);">通过</a>
												<?php break; default: ?>
													<a href="javascript:void(0);" onclick="setstatus(<?php echo $list['id']; ?>,1);">审核中</a>
												
										<?php endswitch; else: switch($list['status']): case "1": ?>通过<?php break; default: ?>审核中
										<?php endswitch; endif; ?>
										
									</td>
									
									<td class="text-center">
										<?php switch($list['status']): case "1": ?>编辑<?php break; default: ?>
											 <a href="javascript:void(0);" onclick="addNew(<?php echo (isset($list['id']) && ($list['id'] !== '')?$list['id']:""); ?>);">编辑</a>
										<?php endswitch; ?>
											<a href="javascript:void(0);" onclick="delRole(<?php echo (isset($list['id']) && ($list['id'] !== '')?$list['id']:""); ?>);">删除</a>	
									</td>
								</tr>  
				     <?php endforeach; endif; else: echo "" ;endif; ?>
 ...
 $(function(){
	...
	function setstatus(id,status){			
			var url = "<?php echo url('setSignStatus'); ?>";	
			$.ajax({
						url : url,
						dataType : "json", 
						type : "post",
           ...
           return; 
     	   var ids;
 	    	...
 	    	if(!ids.length){
     		 ...
     		 $.post("<?php echo url('delSign'); ?>",{'ids':ids},function(data){
     			if (data.code) {
		    		 alert(data.msg);
		    	}else{
		    		 location.reload();
...
<div class="modal fade" id="newModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
    ...
    <script type="text/javascript">
		 ...
		 function addNew(uid){	
			 ...
			 if(uid > 0){
				 		...
				 		$.ajax({
				 				type: "POST",
				 				dataType:'json',
				 				url: "<?php echo url('User/Sms/getSign'); ?>",
				 				cache: false,
				 				data: {id:uid},
				 				success: function(data) {
		 ...
		 function uploadData(){
				...
				var href = "<?php echo url('User/Sms/signature'); ?>";
